...
class Neo4jKnowledgeGraph:
    ...
    def add_relation(self, src, dst, relation):
        with self.driver.session() as session:
            session.run("MATCH (a:Entity {name: $src}), (b:Entity {name: $dst}) MERGE (a)-[r:REL {type: $rel}]->(b)", src=src, dst=dst, rel=relation)

    def query(self, entity):
        with self.driver.session() as session:
            result = session.run("MATCH (e:Entity {name: $name})-->(n) RETURN n.name, n.label", name=entity)
